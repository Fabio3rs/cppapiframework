...
public:
    ...
    auto onJobStarted(const std::string &queue,
        ...
        events.push_back({"STARTED", queue, jobName, jobUuid, job::noerror, tries});
        return startTime;
    }
    ...
    void onJobCompleted(const std::string &queue,
        ...
        events.push_back({"COMPLETED", queue, jobName, jobUuid, result, tries, 0, duration});
    }
    ...
    void onJobRetry(const std::string &queue,
        ...
        events.push_back({"RETRY", queue, jobName, jobUuid, job::errorretry, tries, retryAfterSecs});
    }

    void onJobRemoved(const std::string &queue,
                      const std::string &jobName,
                      const std::string &jobUuid,
                      job::jobStatus finalResult,
                      size_t totalTries) override {
        events.push_back({"REMOVED", queue, jobName, jobUuid, finalResult, totalTries});
    }
};
...
TEST(TestWorkerMetrics, CallbackIsCalledWhenSet) {
    ...
    EXPECT_GE(metricsCallback->events.size(), 3); // QUEUED + STARTED + COMPLETED
    
    // Encontrar evento STARTED
    auto startedIt = std::find_if(metricsCallback->events.begin(), 
                                  metricsCallback->events.end(),
    ...
    auto completedIt = std::find_if(metricsCallback->events.begin(), 
                                    metricsCallback->events.end(),
...
TEST(TestWorkerMetrics, RetryJobCallsRetryCallback) {
    ...
    auto retryIt = std::find_if(metricsCallback->events.begin(), 
                                metricsCallback->events.end(),
...
TEST(TestWorkerMetrics, SimpleConsoleMetricsWorks) {
    ...
    auto startTime = consoleMetrics->onJobStarted("test_queue", "TestJob", "uuid-123", 1);
    
    // Pequena espera para simular processamento
    std::this_thread::sleep_for(std::chrono::milliseconds(10));
    
    consoleMetrics->onJobCompleted("test_queue", "TestJob", "uuid-123", job::noerror, startTime, 1);
