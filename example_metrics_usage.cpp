...
class ExampleJob : public job::QueueableJob {
  ...
  public:
    ...
    ExampleJob() = default;
    ExampleJob(std::string msg, int timeMs)
        : message(std::move(msg)), processTime(timeMs) {}
};
...
class StatisticsMetrics : public job::WorkerMetricsCallback {
    ...
    std::unordered_map<std::string, std::vector<std::chrono::milliseconds>>
        durations;
    size_t totalJobs{0};
  ...
  public:
    void onJobQueued(const std::string &queue, const std::string &jobName,
        ...
        std::cout << "[STATS] Job " << jobName
                  << " enfileirado. Total: " << totalJobs << std::endl;
    }

    auto onJobStarted(const std::string &queue, const std::string &jobName,
                      const std::string &jobUuid, size_t tries)
        -> std::chrono::steady_clock::time_point override {
        std::cout << "[STATS] Iniciando " << jobName << " (tentativa " << tries
                  << ")" << std::endl;
    ...
    void onJobCompleted(const std::string &queue, const std::string &jobName,
        ...
        if (result == job::noerror) {
            ++successfulJobs;
            std::cout << "[STATS] Job " << jobName
                      << " concluído com sucesso em " << duration.count()
                      << "ms" << std::endl;
        } else {
            ++failedJobs;
    ...
    void onJobRetry(const std::string &queue, const std::string &jobName,
        ...
        std::cout << "[STATS] Job " << jobName << " reagendado (tentativa "
                  << tries << ", próxima em " << retryAfterSecs << "s)"
                  << std::endl;
    }
    ...
    void printStatistics() const {
        ...
        for (const auto &[jobName, times] : durations) {
            if (!times.empty()) {
                ...
                std::cout << "  " << jobName << ": " << avg.count()
                          << "ms (média de " << times.size() << " execuções)"
                          << std::endl;
            }
        }
        std::cout << "===================\n\n";
